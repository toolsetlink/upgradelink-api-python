...
try:
    ...
    print("请先安装依赖: pip install alibabacloud-tea alibabacloud-tea-util darabonba-base-python")
    DEPENDENCIES_INSTALLED = False


@unittest.skipIf(not DEPENDENCIES_INSTALLED, "依赖模块未安装，跳过测试")
class TestUpgradeLinkClient(unittest.TestCase):
    
    def setUp(self):
        # 设置测试用的配置
        self.access_key = "mui2W50H1j-OC4xD6PgQag"
        self.access_secret = "PEbdHFGC0uO_Pch7XWBQTMsFRxKPQAM2565eP8LJ3gc"
    ...
    def test_get_file_upgrade(self):
        """获取file应用升级内容"""
        try:
            # 创建客户端
            client = Client(self.config)
            ...
            # 设置请求参数
            file_key = "LOYlLXNy7wV3ySuh0XgtSg"
            version_code = 1
            appoint_version_code = 0
            dev_model_key = ""
            dev_key = ""
